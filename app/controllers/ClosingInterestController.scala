...
class ClosingInterestController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(mode: Mode) = (authenticate andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.closingInterest match {
        case None => form
        case Some(value) => form.fill(value)
      }
      Ok(closingInterest(appConfig, preparedForm, mode))
  }
